...
## 工夫したところ
このアプリを作った目的が、マイナースポーツアスリートの情報共有がテーマなので、どこのページからも必要なページに即座に遷移出来るように、様々な場所にlinkボタンを作りました。また、スポーツテストスコア自動計算機能や、動体視力トレーニング、スポーツニュース閲覧ページなど、自分の技術力向上の為に作成したのはもちろん、ユーザーが少しでも楽しめるうように多数の機能を盛り込みました。
## 今後について
